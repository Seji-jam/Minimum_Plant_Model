...
## I used the new SLA from the P files (avg of max and min) and without anything else the model got fixed!
## what about porting new LAI for potted leaves! 
## what about dynamic SLA in both logistic file and calculating LAI
...
class ModelHandler:
    ...
    def read_input_files(self):
        ...
        # Read greenhouse_mode flag safely
        greenhouse_mode = bool(int(params_dict.get("greenhouse_mode", 0)))  # default is False
        if 'avg_K' not in params_dict:
            n = 500
            phi = np.random.uniform(0, 2*np.pi, n)       # random leaf azimuths
            # K = projection factor for an ellipse relative to its area;
            # for an ellipse with random azimuth we can use:
            Kvals = np.sqrt(np.cos(phi)**2 + (1/params_dict['leaf_len_to_width'])**2 *
                            np.sin(phi)**2)
            params_dict['avg_K'] = Kvals.mean()
            # print(params_dict['avg_K'])
        # print(params_dict)
        # print(greenhouse_mode)
        # Return all required elements
        return drivers, params_dict, resource_pool_params.to_dict(orient='records'), greenhouse_mode

    def plot_outputs(self):
        # Plot carbon pool over time
        plt.figure(figsize=(10, 6))
        plt.plot(self.log_DOY, self.log_carbon_pool, marker='o', linestyle='-')
        plt.xlabel('Time Step (DOY)')
        plt.ylabel('Carbon Pool (g C per plant)')
        plt.title('Carbon Pool over Time')
        plt.grid(True)
        plt.show()
        
        # # Plot RP demand and allocation for each resource pool
        # for rp_name in self.log_rp_demand.keys():
        #     plt.figure(figsize=(10, 6))
        #     plt.plot(self.log_DOY, self.log_rp_demand[rp_name]-self.log_rp_allocation[rp_name], label=f"{rp_name} Demand", linestyle='-')
        #     plt.plot(self.log_DOY, self.log_rp_allocation[rp_name], label=f"{rp_name} Allocation", linestyle='-')
        #     plt.xlabel('Time Step (DOY)')
        #     plt.ylabel('Carbon (g C)')
        #     plt.title(f"Resource Pool '{rp_name}': Demand vs Allocation")
        #     plt.legend()
        #     plt.grid(True)
        #     plt.show()
        ...
        for rp_name in self.log_rp_demand.keys():
            plt.figure(figsize=(10, 6))
            diff = np.array(self.log_rp_demand[rp_name]) - np.array(self.log_rp_allocation[rp_name])
            plt.plot(self.log_DOY, diff, label=f"{rp_name} (Demand - Allocation)", linestyle='-')
            plt.xlabel('Time Step (DOY)')
            plt.ylabel('Difference in Carbon (g C)')
            plt.title(f"Resource Pool '{rp_name}': Demand Minus Allocation")
            plt.legend()
            plt.grid(True)
            plt.show()
        # Plotting lai
        ...
        plt.plot(self.log_DOY, self.log_thermal_age, linestyle='-')
        ...
        # first_key, first_value = next(iter(self.log_resource_pool_sizes.items()))
        # plt.figure(figsize=(8, 6))
        # plt.plot(first_value, label=first_key)
        # plt.xlabel('timestep')
        # plt.ylabel('RP Size')
        # plt.xlim(0, 4000)
        # plt.ylim(0, 19.5)
        ...
        # last_key, last_value = list(self.log_resource_pool_sizes.items())[-1]
        # plt.figure(figsize=(8, 6))
        # plt.plot(last_value, label=last_key)
        # plt.xlabel('timestep')
        # plt.ylabel('RP Size')
        # plt.xlim(0, 4000)
        # plt.ylim(0, 65)
    ...
    def run_simulation(self):
        drivers, self.params_dict, resource_pool_params, greenhouse_mode = self.read_input_files()
        plant_simulated = Plant(self.params_dict, resource_pool_params)
        ...
        for index, row in drivers.iterrows():
            ...
            fractional_DOY   = DOY + hour/24.0          # e.g. 135.75
            self.current_time = fractional_DOY          # store for logging
            ...
            driver_entries = {
                ...
                'radiation'   : row.get('radiation', np.nan),
            ...
            # -------------------------------------- NEW BLOCK ---------------
            # We accept three possible columns in the driver file:
            #   * radiation              – total short‑wave, W m‑2   (regular outside env)
            #   * Incoming_PAR_umol      – PAR, µmol m‑2 s‑1          (greenhouse logger)
            #   * Incoming_PAR_Wm2       – PAR, W m‑2                 (already converted)
            
            if 'Incoming_PAR_Wm2' in row and not np.isnan(row['Incoming_PAR_Wm2']):
                par_direct_Wm2 = row['Incoming_PAR_Wm2']                       # already W m‑2
            elif 'Incoming_PAR_umol' in row and not np.isnan(row['Incoming_PAR_umol']):
                par_direct_Wm2 = row['Incoming_PAR_umol'] * 0.217     # 1 µmol m‑2 s‑1 ≈ 0.217 W m‑2 #convert
            else:
                par_direct_Wm2 = None                                          # outdoor run
            absorbed_par_meas = row.get('Absorbed_PAR_Wm2', np.nan)
            # ---------------------------------------------------------------
            ...
            #         direct_par_input=par_direct_Wm2,greenhouse_mode=greenhouse_mode)          # ← pass the new value
            aboveground_environment_instance.compute_canopy_light_environment(
                    Leaf_Blade_Angle = plant_simulated.get_parameters()['Leaf_Blade_Angle'],
                    Leaf_Area_Index  = plant_simulated.get_leaf_area_index(),
                    direct_par_input = par_direct_Wm2,          # existing (may be None)
                    absorbed_par_input = absorbed_par_meas,     # NEW
    ...
    def save_outputs_to_csv(self, output_filename='model_outputs.csv'):
        sla = self.params_dict.get("Specific_leaf_area")  # m²/g, fallback to 0.23
        ...
        total_leaf_area_cm2 = []
        for rp_name, series in self.log_resource_pool_sizes.items():
            output_df[f"{rp_name}_C"] = series
    
            if rp_name.lower().startswith("l"):
                # Convert to cm² using SLA
                leaf_area_cm2 = np.array(series) * sla * 10_000
                output_df[f"{rp_name}_area_cm2"] = leaf_area_cm2
                total_leaf_area_cm2.append(leaf_area_cm2)
    
            # Add RP demand and allocation
            output_df[f"{rp_name}_demand"] = self.log_rp_demand[rp_name]
            output_df[f"{rp_name}_allocation"] = self.log_rp_allocation[rp_name]
    
        # Total simulated leaf area across RPs
        if total_leaf_area_cm2:
            output_df["Total_leaf_area_cm2"] = np.sum(total_leaf_area_cm2, axis=0)
...
class AbovegroundEnvironment(Environment):
    ...
    #                                   Leaf_Blade_Angle,
    #                                   Leaf_Area_Index,
    #                                   direct_par_input=None,
    #                                   absorbed_par_input=None,      # NEW
    #                                   greenhouse_mode=False):

    #     Scattering_Coefficient_PAR = 0.2
    #     Canopy_Diffuse_Reflection_Coefficient_PAR = 0.057
    
    #     # Choose the correct PAR signal
    #     if direct_par_input is None:
    #         Incoming_PAR = 0.5 * self.exogenous_inputs['radiation']
    #     else:
    #         Incoming_PAR = direct_par_input
    
    #     # Determine light fractions
    #     if greenhouse_mode:
    #         # In greenhouse mode, assume 100% diffuse light
    #         Diffuse_PAR = Incoming_PAR
    #         Direct_PAR = 0.0
    #         Diffuse_Light_Fraction = 1.0
    #     else:
    #         # Outdoor: compute based on atmospheric transmissivity
    #         Atmospheric_Transmissivity = Incoming_PAR / (
    #             0.5 * self.exogenous_inputs['Solar_Constant'] * self.exogenous_inputs['Sin_Beam'])
    ...
    #     Diffuse_Extinction_Coefficient_PAR = self.KDF_Coeff(
    #         Leaf_Area_Index, Leaf_Blade_Angle_Radians, Scattering_Coefficient_PAR)
    ...
    #     print('PAR:',Absorbed_PAR_Sunlit, Absorbed_PAR_Shaded)
    #     if greenhouse_mode:
    #         Sunlit_Fraction = 0.0
    #         Absorbed_PAR_Sunlit = 0.0
    #         Absorbed_PAR_Shaded /= Leaf_Area_Index  # Normalize for canopy
    #     else:
    ...
    #         'VPD': self.exogenous_inputs['VPD']
    #     }
    #     if absorbed_par_input is not None and not np.isnan(absorbed_par_input):
    #         Sunlit_Fraction      = 0.0                # no direct beam in GH
    #         Absorbed_PAR_Sunlit  = 0.0
    #         Absorbed_PAR_Shaded  = absorbed_par_input / max(Leaf_Area_Index, 0.000001)
    #         aboveground_variables = {
    #             'Absorbed_PAR_Sunlit' : Absorbed_PAR_Sunlit,
    #             'Absorbed_PAR_Shaded' : Absorbed_PAR_Shaded,
    #             'Sunlit_fraction'     : Sunlit_Fraction,
    ...
    def compute_canopy_light_environment(
            ...
            Leaf_Area_Index,
            direct_par_input=None,
            absorbed_par_input=None,     
            greenhouse_mode: bool = False
        ):
        """
        Computes absorbed PAR for outdoor (default) or greenhouse conditions.
    
        Parameters
        ----------
        Leaf_Blade_Angle : float   # degrees
        Leaf_Area_Index  : float   # m² leaf m⁻² ground
        greenhouse_mode  : bool    # True → all PAR treated as diffuse
        """
        # ------------------------------------------------------------------
        # constants
        Scattering_Coefficient_PAR            = 0.20   # s
        Canopy_Diffuse_Reflection_Coefficient_PAR = 0.057  # ρd
    
        # ------------------------------------------------------------------
        # print('PAR',Incoming_PAR)
        if direct_par_input is None:
            Incoming_PAR = 0.5 * self.exogenous_inputs['radiation']
        else:
            Incoming_PAR = direct_par_input
        ...
        Solar_Constant        = self.exogenous_inputs['Solar_Constant']

    
        # --- diffuse / direct partition -----------------------------------
        if greenhouse_mode:
            Diffuse_PAR = Incoming_PAR            # glazing scatters everything
            Direct_PAR  = 0.0
        else:
            # Sin_Beam              = self.exogenous_inputs['Sin_Beam']
            # Solar_Constant        = self.exogenous_inputs['Solar_Constant']
            Atmospheric_Transmissivity = (Incoming_PAR /
                                          (0.5 * Solar_Constant * Sin_Beam))
            if Atmospheric_Transmissivity < 0.22:
                Diffuse_Light_Fraction = 1.0
            elif Atmospheric_Transmissivity <= 0.35:
                Diffuse_Light_Fraction = 1 - 6.4 * (Atmospheric_Transmissivity - 0.22) ** 2
            else:
                Diffuse_Light_Fraction = 1.47 - 1.66 * Atmospheric_Transmissivity
            Diffuse_Light_Fraction = max(
        ...
        Leaf_Blade_Angle_Radians       = Leaf_Blade_Angle * np.pi / 180.0
        Direct_Beam_Extinction_Coefficient = self.KDR_Coeff(Sin_Beam,
                                                            Leaf_Blade_Angle_Radians)
        Diffuse_Extinction_Coefficient_PAR = self.KDF_Coeff(
            Leaf_Area_Index, Leaf_Blade_Angle_Radians,
            Scattering_Coefficient_PAR
        )
        (Scattered_Beam_Extinction_Coefficient_PAR,
         Canopy_Beam_Reflection_Coefficient_PAR) = self.REFLECTION_Coeff(
            Scattering_Coefficient_PAR,
            Direct_Beam_Extinction_Coefficient
        )
        ...
        if not greenhouse_mode:
            (Absorbed_PAR_Sunlit,
             Absorbed_PAR_Shaded) = self.LIGHT_ABSORB(
                Scattering_Coefficient_PAR,
                Direct_Beam_Extinction_Coefficient,
                Scattered_Beam_Extinction_Coefficient_PAR,
                Diffuse_Extinction_Coefficient_PAR,
            ...
            Sunlit_Fraction = (1.0 / Direct_Beam_Extinction_Coefficient /
                               Leaf_Area_Index *
                               (1.0 - np.exp(-Direct_Beam_Extinction_Coefficient *
                                             Leaf_Area_Index)))
    
            # convert from ground- to leaf-area basis
            Absorbed_PAR_Sunlit /= Leaf_Area_Index * Sunlit_Fraction
            Absorbed_PAR_Shaded /= Leaf_Area_Index * (1.0 - Sunlit_Fraction)
            # print('Absorbed',Absorbed_PAR_Sunlit,Absorbed_PAR_Shaded)

        # ------------------------------------------------------------------
        # GREENHOUSE condition  --------------------------------------------------
        else:
            # no direct beam → whole canopy “shaded”
            Sunlit_Fraction      = 0.0
            Absorbed_PAR_Sunlit  = 0.0
    
            Canopy_Total_Abs     = (
                (1.0 - Canopy_Diffuse_Reflection_Coefficient_PAR)
                * Diffuse_PAR
                * (1.0 - np.exp(-Diffuse_Extinction_Coefficient_PAR *
                                Leaf_Area_Index))
            )
            Absorbed_PAR_Shaded  = Canopy_Total_Abs / Leaf_Area_Index   # per-leaf-area
...
class Plant:
    ...
    def carry_out_photosynthesis(self, environmental_variables):
        ...
        print(self.__assimilation_sunlit, self.__assimilation_shaded)
    ...
    # def _calc_lai_pot(self, leaf_area_true_m2: float) -> float:
    #        """
    #        Python translation of the BiogeochemicalCycles::calcLAI() routine.
    #        Works entirely in SI units (m, m², rad).
    #        """
    #        # p   = self.__parameters
    #        leaf_len_to_width = self.__parameters['leaf_len_to_width']            # –
    #        Leaf_Blade_Angle = math.radians(self.__parameters['Leaf_Blade_Angle'])
    #        k   = self.__parameters['avg_K']                        # dimensionless
    #        proj_init = self.__parameters['projectedArea_init']     # m²  
    #        # denominator: maximum projected ground area
    #        max_proj_area_m2 = (4.0 * leaf_len_to_width * k * math.sin(Leaf_Blade_Angle)**2) / 10_000.0   # 1/10 000 converts cm²→m²
    #        max_proj_area_m2 = max(max_proj_area_m2, 1.0e-6)                    # avoid /0
    #        # numerator: projected area of all leaves so far
    #        proj_from_leaves_m2 = leaf_area_true_m2 * math.sin(Leaf_Blade_Angle)             # simple projection
    #        sum_proj_area_m2    = proj_init + proj_from_leaves_m2
    #        return sum_proj_area_m2 / max_proj_area_m2
    def _calc_lai_pot(self, max_area, leaf_area_m2: float) -> float:
        """
        Calculates LAI for a potted plant following full method from Appendix S1.
    
        Parameters
        ----------
        leaf_area_m2 : float
            True leaf area (m²) = SLA × total C in leaves.
        ...
        """
        leaf_len_to_width = self.__parameters['leaf_len_to_width']            # dimensionless (length:width ratio)
        Leaf_Blade_Angle = math.radians(self.__parameters['Leaf_Blade_Angle'])  # convert degrees → radians
        projectedArea_init = self.__parameters['projectedArea_init']   # m²
    
        # Step 1: Estimate effective maximum single-leaf area A_max (m²)
        # We use projectedArea_init as a proxy for the first leaf's true area
        # If you know A_max separately you can replace here
        # A_max_m2 = proj_init_m2 / math.sin(ang)  # reverse projection approx
    
        # Step 2: Calculate true leaf length (m) assuming elliptical shape
        Ellipse_leaf_lenght = 2.0 * math.sqrt(leaf_area_m2 * leaf_len_to_width / math.pi)
        Projected_ellipse_leaf_lenght = Ellipse_leaf_lenght * math.sin(Leaf_Blade_Angle)
        Projected_area_single_leaf = math.pi*Projected_ellipse_leaf_lenght*math.sqrt(leaf_area_m2 / (math.pi*leaf_len_to_width))
        
        
        
        Projected_area_single_leaf = projectedArea_init + Projected_area_single_leaf

    
        # Step 2: Calculate true leaf length (m) assuming elliptical shape
        Ellipse_leaf_max_lenght = 2.0 * math.sqrt(max_area * leaf_len_to_width / math.pi)

    
        # Step 3: Projected radius p (m)
        Projected_ellipse_leaf_max_lenght = Ellipse_leaf_max_lenght * math.sin(Leaf_Blade_Angle)
    
        # Step 4: Maximum projected ground area M (m²)
        Max_proj_area_single_leaf = math.pi * Projected_ellipse_leaf_max_lenght**2
    
        Max_proj_area_single_leaf = max(Max_proj_area_single_leaf, 1.0e-10)   # avoid divide-by-zero

        # Step 6: LAI = current projected leaf area / max projected ground area
        Leaf_Area_Index_Single_Leaf = Projected_area_single_leaf / Max_proj_area_single_leaf

        return Leaf_Area_Index_Single_Leaf 
    ...
    def update_leaf_area_index(self):
        """Recompute LAI from all leaf-related RPs each timestep."""

        sla = self.__parameters['Specific_leaf_area']            # m² g⁻¹
        max_leaf_area=self.__parameters['max_leaf_area']  

        if self.__parameters.get('pot_mode', False):   
            leaf_len_to_width = self.__parameters['leaf_len_to_width']            # dimensionless (length:width ratio)
            Leaf_Blade_Angle = math.radians(self.__parameters['Leaf_Blade_Angle'])  # convert degrees → radians
            projectedArea_init = self.__parameters['projectedArea_init']   # m²
            ...
            t=[]
            for rp in self.__resource_pools:
                if rp.name.lower().startswith('l'):
                     leaf_area_m2 = sla * rp.current_size
                     t.append(leaf_area_m2)
                     Ellipse_leaf_lenght = 2.0 * math.sqrt(leaf_area_m2 * leaf_len_to_width / math.pi)
                     Projected_ellipse_leaf_lenght = Ellipse_leaf_lenght * math.sin(Leaf_Blade_Angle)
                     Projected_area_single_leaf = math.pi*Projected_ellipse_leaf_lenght*math.sqrt(leaf_area_m2 / (math.pi*leaf_len_to_width))
                     Leaves_area.append(Projected_area_single_leaf)  
            Projected_area_all_leaves=sum(Leaves_area)
            # max_leaf_area=max(t)
            Total_projected_area = projectedArea_init + Projected_area_all_leaves


            Ellipse_leaf_max_lenght = 2.0 * math.sqrt(max_leaf_area * leaf_len_to_width / math.pi)
            Projected_ellipse_leaf_max_lenght = Ellipse_leaf_max_lenght * math.sin(Leaf_Blade_Angle)
            Max_proj_area = math.pi * Projected_ellipse_leaf_max_lenght**2
            Max_proj_area = max(Max_proj_area, 1.0e-10)   # avoid divide-by-zero
            # print(Leaves_area)

            Leaf_Area_Index = Total_projected_area / Max_proj_area


            
            # POT SCENARIO
            self.__Leaf_Area_Index = Leaf_Area_Index
        else:  
            leaf_c = sum(rp.current_size for rp in self.__resource_pools
                          if rp.name.lower().startswith('l'))      # << naming rule   
            leaf_area_m2 = sla * leaf_c                              # true leaf area
            gnd = self.__parameters['Single_plant_ground_area']  # m² ground plant⁻¹
            self.__Leaf_Area_Index = leaf_area_m2 / gnd
...
working_dir = r'D:\Diane_lab\simple_model\generic_crop_model_project\MPM_local_tests\Brassica'
...
driver_file = 'MPM_driver.csv'

parameter_file =  'parameters_CC.csv'
resource_pool_file ='RP_CC.csv'

# parameter_file =  'parameters_R500.csv'
# resource_pool_file ='RP_R500.csv'
